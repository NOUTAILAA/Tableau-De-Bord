...
function AddRevision() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:5000/api/revision', formData)
      .then(response => {
        ...
        navigate('/revision-list'); // Navigate to revision-list page
      })
      .catch(error => {
        console.error('There was an error adding the revision!', error);
      });
  };

  return (
    <div className="container mt-4">
      <h2>Add a New Revision</h2>
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn btn-primary">Add Revision</button>
      </form>
    </div>
  );
}
